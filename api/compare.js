...
function buildPrompt(userPrompt, retry = false) {
  const strict = retry
    ? 'IMPORTANTE: tu respuesta anterior no fue JSON válido. Devuelve SOLO JSON válido, sin texto extra ni markdown.\n'
    : '';
...
function normalizeStructured(obj) {
  ...
  const optimized_prompt = typeof obj.optimized_prompt === 'string' ? obj.optimized_prompt.trim() : '';
  const short_prompt = typeof obj.short_prompt === 'string' ? obj.short_prompt.trim() : '';
...
function calcBenchmark(ok, clarity, latencyMs) {
  if (!ok) return 0;
  const quality = Math.round(clamp01(clarity) * 100);
...
async function callModelOnce({ model, prompt, apiUrl, apiKey, retry }) {
  ...
  if (!resp.ok) return { ok: false, status, parsed: null, error: `Upstream HTTP ${status}` };
  ...
  if (!parsed) return { ok: false, status, parsed: null, error: 'Invalid structured output' };
...
async function evaluateModel({ model, prompt, apiUrl, apiKey }) {
  ...
  let result;
  try {
    result = await callModelOnce({ model, prompt, apiUrl, apiKey, retry: false });
  } catch (e) {
  ...
  if (!result.ok && result.error === 'Invalid structured output') {
    try {
      result = await callModelOnce({ model, prompt, apiUrl, apiKey, retry: true });
    } catch (e) {
      result = { ok: false, status: result.status || 0, parsed: null, error: e.message || result.error };
    }
  }

  const latency_ms = Date.now() - started;
  const clarity_score = result.ok ? result.parsed.clarity_score : 0;
  const benchmark_score = calcBenchmark(result.ok, clarity_score, latency_ms);
  ...
  return {
    ...
    clarity_score,
    intent: result.ok ? result.parsed.intent : null,
    short_prompt: result.ok ? result.parsed.short_prompt : null,
    optimized_prompt: result.ok ? result.parsed.optimized_prompt : null,
    error: result.error
  };
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const envModels = (process.env.JUNE_MODELS || 'xiaomi/mimo-v2-flash,deepseek/deepseek-v3.2')
      .split(',')
      .map(s => s.trim())
      .filter(Boolean);
    ...
    const maxModels = Math.max(1, Math.min(20, Number(process.env.MAX_COMPARE_MODELS || 10)));
    const models = (reqModels.length ? reqModels : envModels).slice(0, maxModels);
...
'@ | Set-Content -Encoding UTF8 .\api\compare.js"
